...
A GPIO can only be on or off, yet an LED can clearly be *half* bright. The trick is switching fast enough that the eye integrates the result, and Session 11 already told you why you cannot just use a DAC on this chip — the S3 does not have one. PWM through the `ledc` peripheral is how you get an analog-looking output here.

Two parts, one program:

1. **Knob-controlled brightness.** Drive an LED with `ledc` and set its duty cycle from the potentiometer you read in Session 11. Turning the knob dims the LED smoothly across its full range.
...
- Reuse your ADC code from Session 11, copied into this project.
...
| potentiometer wiper | GPIO1 | unchanged from Session 11 |
...
2. Init ADC1 on the potentiometer (Session 11 code)
...
Exercise_1/
├── main/
│   ├── main.c              (required — ledc PWM + fade API, ADC from Session 11)
